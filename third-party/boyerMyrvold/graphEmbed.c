...
void _EmbedBackEdgeToDescendant(graphP theEmbedding, int RootSide,
                                int RootVertex, int W, int WPrevLink);

int _GetNextVertexOnExternalFace(graphP theEmbedding, int curVertex,
                                 int *pPrevLink);
...
void _OrientVerticesInBicomp(graphP theEmbedding, int BicompRoot,
                             int PreserveSigns);
...
void _EmbedBackEdgeToDescendant(graphP theGraph, int RootSide, int RootVertex,
                                int W, int WPrevLink) {
  ...
  if (theGraph->V[parentCopy].fwdArcList == fwdArc) {
    if (theGraph->G[fwdArc].link[0] == fwdArc)
      ...
      theGraph->V[parentCopy].fwdArcList = theGraph->G[fwdArc].link[0];
  }

  theGraph->G[theGraph->G[fwdArc].link[0]].link[1] =
      theGraph->G[fwdArc].link[1];
  theGraph->G[theGraph->G[fwdArc].link[1]].link[0] =
      theGraph->G[fwdArc].link[0];
  ...
  theGraph->G[theGraph->G[RootVertex].link[RootSide]].link[1 ^ RootSide] =
      fwdArc;
...
int _VertexActiveStatus(graphP theEmbedding, int theVertex, int I) {
  ...
  if ((DFSChild = theEmbedding->V[theVertex].separatedDFSChildList) == NIL)
    ...
    leastLowpoint = theEmbedding->V[DFSChild].Lowpoint;
...
int _GetNextVertexOnExternalFace(graphP theEmbedding, int curVertex,
  ...
  if (theEmbedding->G[nextVertex].link[0] !=
      theEmbedding->G[nextVertex].link[1])
...
void _MergeBicomps(graphP theEmbedding) {
  ...
  while (sp_NonEmpty(theEmbedding->theStack)) {
    ...
    if (theEmbedding->extFace[extFaceVertex].link[0] ==
        theEmbedding->extFace[extFaceVertex].link[1])
      theEmbedding->extFace[extFaceVertex]
          .link[Rout ^ theEmbedding->extFace[extFaceVertex].inversionFlag] = Z;
    else
      theEmbedding->extFace[extFaceVertex]
          .link[theEmbedding->extFace[extFaceVertex].link[0] == R ? 0 : 1] = Z;
...
void _RecordPertinentChildBicomp(graphP theEmbedding, int I, int RootVertex) {
  ...
  if (theEmbedding->V[DFSChild].Lowpoint < I)
    ...
    BicompList = LCPrepend(theEmbedding->BicompLists, BicompList, RootId);
...
#define _GetPertinentChildBicomp(theEmbedding, W)                              \
  (theEmbedding->V[W].pertinentBicompList == NIL                               \
       ? NIL                                                                   \
       : theEmbedding->V[W].pertinentBicompList + theEmbedding->N)
...
void _WalkUp(graphP theEmbedding, int I, int W) {
  ...
  while (Zig != I) {
    ...
    if (theEmbedding->G[Zig].visited == I)
      break;
    if (theEmbedding->G[Zag].visited == I)
      break;
...
void _WalkDown(graphP theEmbedding, int I, int RootVertex) {
  int W, WPrevLink, R, Rout, X, XPrevLink, Y, YPrevLink, RootSide,
      RootEdgeChild;
  ...
  for (RootSide = 0; RootSide < 2; RootSide++) {
    ...
    while (W != RootVertex) {
      ...
      if (theEmbedding->V[W].adjacentTo != NIL) {
        ...
        _EmbedBackEdgeToDescendant(theEmbedding, RootSide, RootVertex, W,
                                   WPrevLink);
        theEmbedding->V[W].adjacentTo = NIL;
      }

      /* If there is a pertinent child bicomp, then we need to push it onto the
         stack along with information about how we entered the cut vertex and
         how we exit the root copy to get to the next vertex. */
    ...
    if (sp_IsEmpty(theEmbedding->theStack)) {
      ...
      /* If the bicomp is reduced to having only two external face vertices
          ...
          NOTE: Prior code based on short-circuit edges did not have this
         problem because the root and W would be joined by two separate
         short-circuit edges, so G[W].link[0] != G[W].link[1]. NOTE: We clear
         the flag because it may have been set in W if W previously became part
         of a bicomp with only two ext. face vertices, but then was flipped and
         merged into a larger bicomp that is now again becoming a bicomp with
         only two ext. face vertices. */

      if (theEmbedding->extFace[W].link[0] ==
              theEmbedding->extFace[W].link[1] &&
          WPrevLink == RootSide)
        theEmbedding->extFace[W].inversionFlag = 1;
      else
        theEmbedding->extFace[W].inversionFlag = 0;
    }
...
int gp_Embed(graphP theGraph, int embedFlags) {
  ...
  if (theGraph == NULL)
    return NOTOK;
  if (embedFlags != 0 && embedFlags != EMBEDFLAGS_PLANAR)
    return NOTOK;
