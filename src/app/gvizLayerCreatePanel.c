...
void gvizLayerCreatePanelDraw(void *layerV, const gvizCamera *camera) {
  ...
  /* Defer dropdown draw to after, since active dropdowns must render last. */
  y += ROW_H + GAP;
  ...
  /* Filepath text box (only meaningful when source = FROM_FILE). */
  GuiLabel((Rectangle){px + GAP, y, LABEL_W, ROW_H}, "File path");
  Rectangle pathRect = {px + GAP + LABEL_W, y, PANEL_W - LABEL_W - GAP * 2, ROW_H};
  ...
  if (GuiDropdownBox(srcRect,
                     "Sierpinski (demo);Octahedron (demo);Random tree (demo);From file",
                     &src, p->sourceDropdownEdit))
