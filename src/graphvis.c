...
void runReingoldTilfordAlgorithm(RTTree *tree, size_t root, size_t level) {
  ...
  // consist of the left most child. We merge subtrees one at a time, updating
  // the lMost rMost data of the blob, as well as creating any needed threads
  ...
  for (int i = 1; i < children->count; i++) {
    ...
    while (!lStop && !rStop) {
      ...
      if (currsep < minsep) {
        ...
        ptr = children->arr + (size_t)idx;
        int n = children->arr + i - ptr;
        ...
        // Total unsigned distance covered so far, i.e. from children[0] to r.
        totalDist += (minsep - currsep);
        currsep = minsep;
      }
      ...
      if (graphGetVertexNeighbors(tree->tree, l)->count == 0) {
        ...
        currsep -=
            tree->offsets[l][graphGetVertexNeighbors(tree->tree, l)->count - 1];
        lOffSum +=
            tree->offsets[l][graphGetVertexNeighbors(tree->tree, l)->count - 1];
    ...
    // Set default ancestor values along the right contour of the blob that we
    // get AFTER the merging process. We only need to update values that are on
    // the right contour of the right subtree, as all the values in the blob are
    // assumed to be set.
    ...
    // distance added after merging the right subtree. seperationDelta holds how
  ...
  while (!WindowShouldClose()) {
...

    {
      ...
      if (state.stack) {
        for (int k = state.stack->count; k > 0; k--) {
          ...
          for (size_t i = 0; i < currGraph->count; i++) {
            ...
            for (int j = 0; j < graphGetVertexNeighbors(currGraph, i)->count;
                 j++) {
              drawEdge(curr,
                       state.gamma.components
                           [currGraph->map[graphGetVertexNeighbors(currGraph, i)
                                               ->arr[j]]],
      ...
      for (size_t i = 0; i < state.g->count; i++) {
        ...
        drawVertex(curr, currOpacity);
        for (int j = 0; j < graphGetVertexNeighbors(state.g, i)->count; j++) {
          drawEdge(curr,
                   state.gamma
                       .components[graphGetVertexNeighbors(state.g, i)->arr[j]],
...
void cleanUpReingoldTilford(RTTree *rtTree, size_t root) {
  ...
  for (int i = 0; i < graphGetVertexNeighbors(rtTree->tree, root)->count; i++) {
    cleanUpReingoldTilford(rtTree,
                           graphGetVertexNeighbors(rtTree->tree, root)->arr[i]);
  }
}
