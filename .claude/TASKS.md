...
- [ ] In `include/app/gvizLayerCreatePanel.h`:
  - [ ] Add `gvizDemoGraphType` enum: `SIERPINSKI_TRI`, `SIERPINSKI_TET`,
        `CARPET`, `RECT_MESH`, `TET_MESH`, `EQ_TRI_MESH`, `KNOTTED_RECT`,
        `MOBIUS`, `KLEIN`, `RANDOM_TREE`.
  - [ ] Add fields to `gvizLayerCreateParams`: `gvizDemoGraphType graphType`,
        `int graphParam1`, `int graphParam2`, `int embDim`.
  - [ ] Defaults: `graphType=SIERPINSKI_TRI`, `graphParam1=3`,
        `graphParam2=3`, `embDim` set by mode (2 for 2D, 3 for 3D) at
        panel init.
- [ ] In `gvizLayerCreatePanelInit`, initialize the new fields.

### Saga B.2: Panel UI — Demos sub-form
- [ ] In `src/app/gvizLayerCreatePanel.c`:
  - [ ] Add a `graphTypeDropdownEdit` field to the panel struct (header).
  - [ ] When source dropdown is "Demos" (renamed from per-demo entries),
        render a `GuiDropdownBox` for graph type with all 10 entries.
  - [ ] Per type, render 0–2 `GuiSpinner` controls labelled appropriately
        (e.g. depth for Sierpinski; L, W for rect mesh; rows, cols for
        Möbius/Klein). Show only inputs relevant to the chosen type.
  - [ ] Add an "Embedding dim" `GuiSpinner` (min 2, max 8). Clamp on edit.
  - [ ] Drop the obsolete demo entries from the Source dropdown — collapse
        to: "Demos;From file".
- [ ] Adjust `PANEL_H` upward to fit new rows; recompute layout offsets.

### Saga B.3: Dispatch demo graph type → generator
- [ ] In `src/app/gvizLayerCreate.c::loadGraphForSource`:
  - [ ] Replace the per-source switch arms for the old demo enums with a
        single `GVIZ_SRC_DEMO` arm that switches on `params->graphType` and
        calls the right `utils/graphs.h` generator with `graphParam1` /
        `graphParam2`.
  - [ ] For `RANDOM_TREE`, keep the existing local `buildRandomTree`
        helper, parameterized by `graphParam1` (max depth).
- [ ] Remove `GVIZ_SRC_DEMO_SIERPINSKI`, `GVIZ_SRC_DEMO_OCTAHEDRON`,
      `GVIZ_SRC_DEMO_RANDOM_TREE` from the source enum (header).
- [ ] Drop `buildOctahedronGraph` (replaced by parametric generators) or
      keep it only if a test still references it (verify via grep first).

### Saga B.4: Plumb `embDim` into GRIP layer init
- [ ] In `include/app/gvizLayerGRIPLive.h`, expose
      `gvizLayerGRIPLiveInitWithDim` (currently file-static) so the mux can
      call it with arbitrary dim.
- [ ] In `src/app/gvizLayerCreate.c::buildGRIPLayer`, when algo is GRIP
      pass `params->embDim` into `gvizLayerGRIPLiveInitWithDim`. Camera
      branch: if `embDim >= 3` use 3D camera (already handled inside the
      init); otherwise 2D.
- [ ] Note: only GRIP supports `embDim > 3`. Tutte / PolyTutte / RT keep
...
- [ ] In `include/renderer/layers/gvizGraphVBO.h`:
  - [ ] Add `int drawDim` field to `gvizGraphVBO` (2 or 3; defaults to 2).
  - [ ] Add `double *projScratch` and `size_t projScratchCap` for PCA
        scratch (interleaved `drawDim`-vectors per vertex).
  - [ ] Add setter `gvizGraphVBOSetDrawDim(gvizGraphVBO *, int)`.
- [ ] In `gvizGraphVBOInit` / `gvizGraphVBORelease`, init/free new fields.
- [ ] Caller plumb: `gvizLayerGRIPLiveInitWithDim` calls
      `gvizGraphVBOSetDrawDim(&self->vbo, dim==3 ? 3 : 2)` after init.

### Saga B.6: PCA projection helper
- [ ] New file `src/utils/gvizPCA.c` + `include/utils/gvizPCA.h`:
  - [ ] `int gvizPCAProject(const double *X, size_t N, size_t embDim,
...
- [ ] CMake: confirm `LAPACK::LAPACK` (or whatever existing target the GRIP
...
- [ ] In `src/renderer/layers/gvizGraphVBO.c`:
  - [ ] Modify `buildExpandedVerts` (and the analogous disc-position path
  ...
  - [ ] When `embDim <= drawDim`, keep the current direct-read fast path.
- [ ] `gvizVertexDiscVBO` similarly needs to read projected positions when
...
- [ ] DiscVBO change: extend its position-upload entry points to accept a
...
- [ ] No extra work — `gvizLayerGRIPLiveUpdate` already bumps `gpuDirty=1`,
...
### Saga B.9: Smoke
- [ ] Build passes.
- [ ] Manual: open Demos panel, pick Möbius strip 8x16 with embDim=5 in a
      3D scene; confirm GRIP renders as a 3D PCA projection that converges.
- [ ] Manual: pick rect mesh with embDim=2 in a 2D scene; confirm fast
